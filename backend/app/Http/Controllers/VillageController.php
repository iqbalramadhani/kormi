...
class VillageController extends BaseController
{
    ...
    public function index(Request $request){
        $query = $this->model->select('villages.*','districts.name as districts_name')->join('districts','villages.district_id','districts.id');
    ...
    public function show($id){
        $data = $this->model->with('District')->where('id',$id)->first();
        if($data){
            return Response::api("Success",$data);
        }             
