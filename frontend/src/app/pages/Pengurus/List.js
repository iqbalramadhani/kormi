...
export function PengurusList(props) {
  ...
  const getData = async () => {
    let { data, error } = await PengurusService.list({
      page,
  ...
  const handleDelete = async (id) => {
    ShowAlert.confirm().then(async (result) => {
      if (result.value) {
        let { data, error } = await PengurusService.delete(id);
        if (data) {
          getData();
        }
        if (error) {
  ...
  return (
    <Card>
      <CardHeader title='Daftar Pengurus Kormi'>
        ...
        <div className='row'>
          ...
          <div className='col-md-3'>
            <div className='d-flex mb-6 float-right'>
              ...
              <Link className='btn btn-success mr-2' to={'/pengurus/create'}>
                Tambah
              </Link>
            </div>
          </div>
