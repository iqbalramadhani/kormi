...
export function CmsAdministratorEditPage(props) {
  ...
  const updateCmsAdministrator = async () => {
    let { data, error } = await CmsAdministratorServices.edit(formData, null, formData.id);
    if (data) {
    ...
    <>
      {previousPage.redirect ? (
        ...
        <Card>
          ...
          >
            <CardHeader title="Perbarui Admin">
              <CardHeaderToolbar>
                <button
                  type="button"
...
export default CmsAdministratorEditPage;
