...
export function CommentEventList(props) {
  ...
  const getData = async () => {
    let { data, error } = await EventServices.comments(props.match.params.id);

    console.log(data);
    if (data) {
      setData(data.record);
      setTotalData(data.specific_count.show);
    }
    if (error) {
  ...
  return (
    <Card>
      <CardHeader title='Komentar Acara'>
        <CardHeaderToolbar>
